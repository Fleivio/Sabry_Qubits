...
observeLeftR (QR ptr) = do
  qVal <- readIORef ptr
  let leftProb a =
        sqrt . sum $ [squareModulus (getProb qVal (a, b)) :+ 0 | b <- basis]
      leftQval = mkQV [(a, leftProb a) | a <- basis]
  observResult <- observeV leftQval
  let nv =
        mkQV [((observResult, b), getProb qVal (observResult, b)) | b <- basis]
...
observeRightR (QR ptr) = do
  qVal <- readIORef ptr
  let rightProb a =
        sqrt . sum $ [squareModulus (getProb qVal (b, a)) :+ 0 | b <- basis]
      rightQval = mkQV [(a, rightProb a) | a <- basis]
  observResult <- observeV rightQval
  let nv =
        mkQV [((b, observResult), getProb qVal (b, observResult)) | b <- basis]
  writeIORef ptr (normalize nv)
  return observResult
